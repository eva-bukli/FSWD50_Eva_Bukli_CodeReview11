    ...
    <div class="card h-100">
      ...
      <div class="card-body">
        ...
        <form class="mb-1" id="single" action='home.php' method='get' >
          <button name="like" value="<?php echo $row["concert_id"]; ?>" class="btn mb-2 btn-primary "><i class="fa fa-thumbs-up"></i> <?php echo $row["concert_like"]; ?></button>
        </form>
        <form id="single" action='single_concert.php' method='get' >
          <button class="btn btn-info btn-block  mb-2" type="submit" name='idc' value="<?php echo $row["concert_id"]; ?>">More..</button>  
        </form><?php  
 if (1==$currentrole) { echo 
        "<form id='single' action='cupdate.php' method='get' >
            <button class='btn btn-warning btn-block  mb-2' type='submit' name='editcon' value='".$row['concert_id']."' >Edit</button>
        </form>
        <form id='single' action='home.php' method='post' >
            <button class='btn btn-danger btn-block  mb-2' type='submit' name='deletecon' value='".$row['concert_id']."' >Delete</button>
        </form>"; }?>
      </form>
    </div>
  </div>
